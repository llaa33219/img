...
document.addEventListener('paste', (event) => {
  ...
  if (mediaFile) {
    ...
    if (!allowedImageExtensions.includes(fileExtension) && !allowedVideoExtensions.includes(fileExtension)) {
      statusElement.innerText = "Unsupported file format.";
      return;
    }
    ...
    statusElement.innerText = "Media selected from clipboard.";
  } else {
    statusElement.innerText = "No image or video found in clipboard.";
  }
});
  ...
  async function generateVariantsAndAnalyze(base64Image) {
    ...
    for (const variant of variants) {
      try {
        ...
        console.error("Failed to analyze transformed image:", e);
      }
    }
    return results;
  }
      ...
      try {
        ...
        for (const result of analysisResults) {
          ...
          if (safeSearch) {
            if (censorIfPossibleOrAbove.includes(safeSearch.adult)) {
              foundSuspicious = true; 
                reasons.push('Adult/Racy content detected');
            }
            if (censorIfPossibleOrAbove.includes(safeSearch.racy)) {
              foundSuspicious = true;
                reasons.push('Possible racy content detected');
            }
            if (censorIfPossibleOrAbove.includes(safeSearch.violence)) {
              foundSuspicious = true;
                reasons.push('Possible violent content detected');
            }
          }
          ...
          // 라벨에서 정치/비하/잔인/성적 키워드 감지 시 POSSIBLE 수준에서도 검열
            ...
            foundSuspicious = true; reasons.push('Political content detected');
            }
            if (checkKeywords(labelDescriptions, derogatoryKeywords)) {
            foundSuspicious = true; reasons.push('Derogatory content detected');
            }
            if (checkKeywords(labelDescriptions, cruelKeywords)) {
            foundSuspicious = true; reasons.push('Cruel content detected');
            }
            if (checkKeywords(labelDescriptions, sexualKeywords)) {
            foundSuspicious = true; reasons.push('Sexual content detected');
            }
        ...
        statusElement.innerText = "An error occurred while analyzing the image.";
        reject(error);
      }
    };
...
async function analyzeVideoFrames(videoFile) {
  return new Promise((resolve, reject) => {
    ...
    video.addEventListener('loadeddata', async () => {
      ...
      if (censored) {
        resolve({ isCensored: true, reasons: ['Video frame censorship'] });
      } else {
  ...
  try {
    for (const mediaFile of mediaFiles) {
      ...
      if (fileNameParts.length < 2) {
        statusElement.innerText = "The file has no extension.";
        return;
      }
      ...
      if (isVideo) {
        statusElement.innerText = "Analyzing video frames...";
    ...
    if (uploadedNames.length > 0) {
      const imageUrl = `https://github.bloupla.net/img/?=${uploadedNames.join(',')}`;
    ...
    if (skippedFiles.length > 0) {
      ...
      statusElement.innerText = `Inappropriate content detected: ${skippedList}`;
    }
  } catch (error) {
    console.error("Error occurred during upload:", error);
    statusElement.innerText = "An error occurred during upload.";
  } finally {
    if (isAnyVideo) {
      unmuteSiteSounds();
    }
...
async function loadImageFromUrl() {
  ...
  if (imageNames.length > 0) {
    ...
    try {
      ...
      statusElement.innerText = "An error occurred while loading the image.";
    }
  ...
  try {
    ...
    if (isImage || isVideo) {
      if (needsCensorship) {
        ...
        censorshipMessage.innerText = "This content has been censored due to inappropriate content.";
    ...
    statusElement.innerText = "An error occurred while loading the media.";
  }
}
...
document.getElementById('copyImage').addEventListener('click', async () => {
  ...
  try {
    ...
    alert('Image/Video link has been copied to clipboard!');
  } catch (error) {
